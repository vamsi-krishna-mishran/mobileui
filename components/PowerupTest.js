...
function PowerupTest()
{
    ...
    const upload = async () =>
    {
        if (data.length == 0)
        {
            setErr(prev => ({ ...prev, data: "Table should not be empty", state: false }))
            setSnack(true);
        }
        else if (data.some(el => el.some(el2 => el2 == "" || el2 == false)))
        {
            setErr(prev => ({ ...prev, data: "Table Fields should not be empty.", state: false }))
            setSnack(true);
        }
        else if (desc == "" || desc == false)
        {
            setErr(prev => ({ ...prev, data: "Description  should not be empty", state: false }))
            setSnack(true);
        }
        else
        {
    ...
    const uploadData = async () =>
    {
        ...
        let sendData = dataAr.map(el => ({ Ref: el[0], PUT: el[1], Exp: el[2], Mes: el[3] }));
        setSnack(true);
        let res = await fetch('https://webapifluent20230616160256.azurewebsites.net/api/PowerUp', {
            method: "POST",
            headers: {
    ...
    useEffect(() =>
    {
        (
            async () =>
            {
                ...
                // })
                let res = await fetch('https://webapifluent20230616160256.azurewebsites.net/api/PowerUp?Id=1');
                if (res.status === 200)
                {
        ...
        <ScrollView>
            ...
            <View style={{ marginRight: 20, marginTop: 5 }} alignItems="flex-end">
                <TouchableOpacity disabled={edit} onPress={AddValues}>
                    <Ionicons style={{ color: "blue", fontSize: 40 }} name="add-circle-outline"></Ionicons>
                </TouchableOpacity>
            </View>
...
const DattaTable = ({ data, setData, edit }) => (
    ...
    <View style={{ marginTop: 20 }}>
        <Card style={{ paddingBottom: 10 }} mode='elevated' width="96%" alignSelf="center">
            <HStack justifyContent="space-between" py="5">
                <Text style={{ flex: 5, paddingLeft: 10, fontWeight: 'bold' }}>Ref Des</Text>
            ...
            <Divider />
            {data.map((element, index) => (<MyComponent dataAr={data[index]} setDataAr={(modArr) => setData(prev => { let up = [...prev]; up[index] = modArr; return up })} edit={edit} key={index} />))}
        </Card>
