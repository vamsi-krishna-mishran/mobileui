...
function CameraPicture()
{
    ...
    const uploadBoardData = async () =>
    {
        ...
        let res = await fetch('https://webapifluent20230616160256.azurewebsites.net/api/BareBoard', {
            ...
            body: JSON.stringify({ ImageName: "image temp from native", BoardType: "CameraPicture", "Description": desc, "ImageData": image, IId: 1 })
    ...
    useEffect(() =>
    {
        (
            async () =>
            {
                ...
                // })
                let res = await fetch('https://webapifluent20230616160256.azurewebsites.net/api/BareBoard?IBoardType=CameraPicture&Id=1');
                if (res.status === 200)
                {
    ...
    return (
        (<ScrollView>
            ...
            <KeyboardAvoidingView>
                ...
                <Image style={{ marginTop: 10, borderRadius: 20, alignSelf: 'center', width: 330, height: 350 }} source={image ? { uri: `data:image/png;base64,${image} ` } : require('../assets/notfound.jpg')} />
                <HStack justifyContent="center" space={5}>
                    <AddImage setImage={setImage} edit={edit} />
                    <Button style={{ height: 40, marginTop: 5 }} mode="contained" icon="delete" disabled={edit} onPress={() => setImage(false)} >Delete</Button>
                </HStack>
                <Description edit={edit} desc={desc} setDesc={setDesc} />
                <HStack justifyContent="center" m={2} space={3}>

                    <TouchableOpacity>
                        <Button mode="contained" name='save' disabled={edit} onPress={uploadBareboard} > Save</Button>
                    </TouchableOpacity>
                    <TouchableOpacity>
