...
function EnvironmentalTest() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // })
      let res = await fetch(
        `${API_URL}/api/BareBoard?IBoardType=Environmental&Id=1`
      );
      if (res.status === 200) {
  ...
  const upload = async () => {
    if (data.some((el) => el == "" || el == false)) {
      setErr((prev) => ({
        ...prev,
        data: "Ambient, Sys, FPA should not be empty",
        state: false,
      }));
      setSnack(true);
      ...
      setErr((prev) => ({
        ...prev,
        data: "Remark should not be empty",
        state: false,
      }));
      setSnack(true);
    } else {
    ...
    <ScrollView>
      ...
      >
        <HStack justifyContent="space-between" p="5">
          ...
          <Text style={{ flex: 5, fontWeight: "bold" }} disabled={edit}>
            SYS
          </Text>
          <Text style={{ flex: 5, fontWeight: "bold" }} disabled={edit}>
            FPA
